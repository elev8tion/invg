-- Multi-Business Invoice Management System Schema
-- Supports multiple businesses, payment tracking, PO management, email integration, and more
...
BEGIN
    ...
    -- Calculate discount
    NEW.discount_amount = NEW.subtotal * (NEW.discount_rate / 100);
    ...
    ELSIF NEW.due_date < CURRENT_DATE AND NEW.balance_due > 0 AND NEW.status NOT IN ('draft', 'cancelled') THEN
        NEW.status = 'overdue';
    END IF;
